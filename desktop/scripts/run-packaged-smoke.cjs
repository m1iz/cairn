...
const tempRoot = mkdtempSync(join(tmpdir(), 'cairn-packaged-smoke-'))
...
function resolveAppPath(argv) {
  ...
  const envPath = String(process.env.CAIRN_SMOKE_APP || '').trim()
  ...
  if (process.platform === 'darwin') {
    ...
    candidates = [join(distRoot, 'linux-unpacked', 'cairn')]
  }
...
function smokeEnvironment() {
  const env = {
    ...
    CAIRN_CONFIG_DIR: stateRoot,
    CAIRN_BUILD_COMMIT: readGitCommit(repoRoot),
