...
describe('trusted release bundle assembler', () => {
  it('assembles exactly seven artifacts after validating every receipt', () => {
    ...
    ).toContain('Cairn-Agent-0.1.0-win-x64.exe')
  })
  ...
  it('rejects a candidate whose platform checksum does not match', () => {
    ...
    writeFileSync(
      ...
      `${'0'.repeat(64)} *Cairn-Agent-0.1.0-win-x64.exe\n`,
    )
...
function createFixture(): { root: string; input: string; output: string } {
  const root = mkdtempSync(join(tmpdir(), 'cairn-release-bundle-'))
  roots.push(root)
  ...
  const artifactGroups = {
    'SHA256SUMS-macos-arm64.txt': [
      'Cairn-Agent-0.1.0-mac-arm64.dmg',
      'Cairn-Agent-0.1.0-mac-arm64.zip',
    ],
    'SHA256SUMS-macos-x64.txt': [
      'Cairn-Agent-0.1.0-mac-x64.dmg',
      'Cairn-Agent-0.1.0-mac-x64.zip',
    ],
    'SHA256SUMS-windows-x64.txt': ['Cairn-Agent-0.1.0-win-x64.exe'],
    'SHA256SUMS-linux-x64.txt': [
      'Cairn-Agent-0.1.0-linux-x64.AppImage',
      'Cairn-Agent-0.1.0-linux-x64.deb',
    ],
  }
