...
describe('CycloneDX SBOM merger', () => {
  it('connects the desktop Core component to the Core lockfile graph', () => {
    ...
    const desktopCore = merged.dependencies.find(
      (dependency) => dependency.ref === 'desktop|@cairn/core',
    )
...
function createFixture() {
  const root = mkdtempSync(join(tmpdir(), 'cairn-sbom-merge-'))
  roots.push(root)
  ...
  const desktopNames = dependencyNames(desktopManifest).filter(
    (name) => name !== '@cairn/core',
  )
  const coreComponents = [component('@cairn/core', 'core')]
  coreComponents.push(...coreNames.map((name) => component(name, 'core')))
  const desktopComponents = [component('@cairn/core', 'desktop')]
  ...
  writeFileSync(
    ...
    JSON.stringify(
      bom('desktop-root', desktopComponents, [
        ...
        { ref: 'desktop|@cairn/core', dependsOn: [] },
