...
describe('unsigned preview release channel', () => {
  ...
  it('uses an isolated unsigned builder configuration for all seven files', () => {
    ...
    expect(config).toMatchObject({
      ...
      linux: {
        target: ['AppImage', 'deb'],
        artifactName:
          'Cairn-Agent-${version}-UNSIGNED-PREVIEW-linux-x64.${ext}',
      },
    })
  ...
  it('writes a fail-closed unsigned candidate receipt with artifact hashes', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'cairn-preview-'))
    const dist = path.join(root, 'dist')
    const tag = 'v0.1.0-preview.1'
    const commit = 'a'.repeat(40)
    const runId = '123456'
    const artifactNames = [
      'Cairn-Agent-0.1.0-preview.1-UNSIGNED-PREVIEW-linux-x64.AppImage',
      'Cairn-Agent-0.1.0-preview.1-UNSIGNED-PREVIEW-linux-x64.deb',
    ]
  ...
  it('rejects internal markers and Stable artifact names as candidate input', () => {
    ...
    fs.writeFileSync(
      path.join(dist, 'Cairn-Agent-0.1.0-linux-x64.AppImage'),
      'stable\n',
    )
