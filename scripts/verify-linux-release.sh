...
find_one 'Cairn-Agent-*-linux-x64.AppImage' APPIMAGE
find_one 'Cairn-Agent-*-linux-x64.deb' DEB
...
prepare() {
  ...
  local commit="${CAIRN_RELEASE_COMMIT:-${GITHUB_SHA:-$(git -C "$ROOT" rev-parse HEAD)}}"
  mkdir -p "$receipt_dir"
...
smoke() {
  ...
  while IFS= read -r candidate; do
    if [[ -f "$candidate" && -x "$candidate" && "$(basename "$candidate")" == 'cairn' ]]; then
      installed_executable="$candidate"
      break
    fi
  done < <(dpkg -L "$DEB_PACKAGE")
  if [[ -z "$installed_executable" ]]; then
    echo "Installed Cairn executable was not found in the DEB file list" >&2
    exit 1
  fi
