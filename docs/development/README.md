...
跨层改动请使用[扩展 Cairn](extending-cairn.md)的同步清单，不要只修改最先报错的一层。

## 数据与测试隔离

运行态数据默认写入 `~/.cairn`。测试必须使用临时 `stateRoot`，不能读取或覆盖开发者的真实模型配置、会话、记忆和凭证。Build workspace 也不能承载 session、附件或 Goal 私有数据。

不要提交 `memory/`、`sessions/`、`.cairn/`、`.team/`、本地配置、`.env`、`node_modules`、构建目录、screenshots 或 test results。完整清单见 [AGENTS.md](../../AGENTS.md)。
