...
> 文档状态：Active<br>
> 面向读者：第一次安装和配置 Cairn 的用户<br>
> 最后核验：2026-08-29<br>
> 事实源：源码构建说明、模型设置页、Session/Project 创建入口

## 1. 安装

1. 按 README 的“快速开始”从源码启动，或使用本地构建脚本生成安装包。
2. 本地安装包已经包含 Electron/Node runtime，不需要另外安装 Python。
3. 安装并启动应用。

可以按 [README 的快速开始](../../README.md#快速开始)启动开发版。
...
会话、记忆、模型配置和附件保存在本机；默认私有数据根是 `~/.cairn`。以下操作仍会把必要内容发送到外部：
