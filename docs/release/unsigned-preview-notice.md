...
3. 可使用 `gh attestation verify <file> --repo <owner>/<repo>` 验证 GitHub 构建来源。
4. Attestation 只证明构建来源和完整性，不代表 Apple/Microsoft 发布者签名，也不会把本版本变成 Stable。

## macOS 单应用确认 / Per-app confirmation

首次打开并看到系统拦截后，确认 SHA-256 无误，再进入 **System Settings → Privacy & Security → Open Anyway**，仅为 Cairn 创建例外。Apple 的风险说明与官方步骤见 [Safely open apps on your Mac](https://support.apple.com/en-us/102445)。不要更改整机 Gatekeeper 安全策略。

After the first blocked launch, verify the SHA-256 digest, then use **System Settings → Privacy & Security → Open Anyway** to create an exception only for Cairn. Keep system-wide Gatekeeper protections enabled.
