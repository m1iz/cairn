...
When creating a new skill from scratch, call the built-in `manage_skill` tool. Cairn writes only to the private user Skill directory; do not choose an output path or create files in the signed built-in directory.
...
After initialization, customize `SKILL.md` and add resources as needed. Use `{{skill_dir}}` for paths to bundled files. Cairn expands this placeholder only when loading the Skill and leaves the file on disk unchanged.
