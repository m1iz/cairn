...
const EXPECTED_OPERATIONS = [
  'agentDefinitions.delete',
  'agentDefinitions.get',
  'agentDefinitions.save',
  'attachments.rawPath',
  'attachments.save',
  'bootstrap',
...
describe('CoreApi', () => {
  ...
  it('binds an approved Goal Plan before resuming the control turn', async () => {
    const root = tmp('cairn-core-api-goal-plan-')
  ...
  it('does not start Goal Plan approval when durable quarantine preparation fails', async () => {
    const root = tmp('cairn-core-api-goal-plan-prepare-fault-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
  ...
  it('aborts and compensates a Goal approval that fails inside approve()', async () => {
    const root = tmp('cairn-core-api-goal-plan-approve-fault-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
  ...
  it('preflights a Goal Plan before approval can issue tokens or create step tasks', async () => {
    const root = tmp('cairn-core-api-goal-plan-preflight-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
  ...
  it('starts with recoverable defaults when Model and MCP configs are corrupt', async () => {
    const root = tmp('cairn-core-api-corrupt-config-')
    const stateRoot = tmp('cairn-core-api-corrupt-state-')
  ...
  it('exposes explicit MCP connection status in the operation, bootstrap, and diagnostics', async () => {
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-mcp-status-'),
      stateRoot: tmp('cairn-core-api-mcp-status-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(new FakeProvider()),
    })
  ...
  it('exposes a typed in-process method for every retired web route operation', async () => {
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-'),
      stateRoot: tmp('cairn-core-api-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(new FakeProvider()),
    })
  ...
  it('lists, previews, and explicitly rewinds file checkpoints without accepting a renderer path', async () => {
    const root = tmp('cairn-core-api-file-checkpoints-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
  ...
  it('boots, submits a chat turn, and persists session runtime state without HTTP', async () => {
    ...
    expect(
      existsSync(
        join(root, '.cairn', 'sessions', activeSessionId, 'history.jsonl'),
      ),
    ).toBe(true)
  ...
  it('starts and controls a Goal through typed CoreApi operations and bootstrap', async () => {
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-goals-'),
      stateRoot: tmp('cairn-core-api-goals-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(new FakeProvider()),
    })
  ...
  it('materializes a draft before Goal creation and fences cross-session reads', async () => {
    const events: Array<Record<string, unknown>> = []
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-goal-draft-'),
      stateRoot: tmp('cairn-core-api-goal-draft-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(new FakeProvider()),
  ...
  it('pauses Goals before archive and cancels, settles, then deletes Goal state with the session', async () => {
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-goal-session-lifecycle-'),
      stateRoot: tmp('cairn-core-api-goal-session-lifecycle-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(new FakeProvider()),
    })
  ...
  it('rejects deleting the only persisted session before mutating its Goal', async () => {
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-last-session-delete-'),
      stateRoot: tmp('cairn-core-api-last-session-delete-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(new FakeProvider()),
    })
  ...
  it('keeps a Goal recoverable when the session store rejects deletion', async () => {
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-session-delete-rejected-'),
      stateRoot: tmp('cairn-core-api-session-delete-rejected-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(new FakeProvider()),
    })
  ...
  it('forwards renderer attachment and requested skill fields to ChatService', async () => {
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-chat-fields-'),
      stateRoot: tmp('cairn-core-api-chat-fields-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(new FakeProvider()),
    })
  ...
  it('stores new private runtime state under .cairn and reports effective paths', async () => {
    const root = tmp('cairn-core-api-state-root-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
    ...
    expect(diagnostics.paths).toMatchObject({
      runtimeRoot: root,
      stateRoot: join(root, '.cairn'),
      sessionsRoot: join(root, '.cairn', 'sessions'),
      memoryRoot: join(root, '.cairn', 'memory'),
    })
  ...
  it('migrates private state from an explicit legacy runtime without changing signed runtimeRoot', async () => {
    const signedRuntimeRoot = tmp('cairn-core-api-signed-runtime-')
    const legacyRuntimeRoot = tmp('cairn-core-api-legacy-runtime-')
    const stateRoot = tmp('cairn-core-api-legacy-state-')
  ...
  it('keeps all user writes in stateRoot when runtime resources are read-only', async () => {
    const runtimeRoot = tmp('cairn-core-api-readonly-runtime-')
    const legacyRuntimeRoot = join(
      tmp('cairn-core-api-readonly-legacy-'),
      'runtime',
    )
    const stateRoot = tmp('cairn-core-api-readonly-state-')
  ...
  it('manages hooks config, audit, and test runs through CoreApi', async () => {
    const root = tmp('cairn-core-api-hooks-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
  ...
  it('publishes hooks metadata, pure validation, matching, and optimistic revisions', async () => {
    const root = tmp('cairn-core-api-hooks-contract-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
  ...
  it('requires confirmation and exact matching selection for hook test execution', async () => {
    const root = tmp('cairn-core-api-hook-test-run-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
  ...
  it('blocks hook test execution in Plan mode before command or audit side effects', async () => {
    const root = tmp('cairn-core-api-hook-plan-guard-')
  ...
  it('blocks hook test execution while an Ask is pending before command or audit side effects', async () => {
    const root = tmp('cairn-core-api-hook-pending-guard-')
  ...
  it('trusts only the active canonical project at its current hooks digest', async () => {
    const root = tmp('cairn-core-api-hook-trust-')
    const stateRoot = join(root, '.cairn-state')
  ...
  it('filters and pages hook audit records and reports missing cancellation targets', async () => {
    const root = tmp('cairn-core-api-hook-audit-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
  ...
  it('returns an async hook run id that can be cancelled exactly once', async () => {
    const root = tmp('cairn-core-api-hook-cancel-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
  ...
  it('runs ConfigChange before config, MCP, and model writes', async () => {
    const root = tmp('cairn-core-api-config-change-')
  ...
  it('saves model config through stateRoot rather than runtimeRoot', async () => {
    const root = tmp('cairn-core-api-model-runtime-')
    const stateRoot = tmp('cairn-core-api-model-state-')
    const api = await CoreApi.create({
      root,
      stateRoot,
      templatesDir: TEMPLATES_DIR,
  ...
  it('rejects draft session ids at bootstrap before activating or replaying', async () => {
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-bootstrap-session-'),
      stateRoot: tmp('cairn-core-api-bootstrap-session-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(new FakeProvider()),
    })
  ...
  it('replays runtime events for the requested session only', async () => {
    const root = tmp('cairn-core-api-runtime-replay-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
  ...
  it('compacts high-frequency delta events in replay by default without touching disk', async () => {
    const root = tmp('cairn-core-api-replay-compact-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
  ...
  it('stamps active turn tasks with their session id and exposes them at bootstrap', async () => {
    ...
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-active-session-'),
      stateRoot: tmp('cairn-core-api-active-session-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(provider),
    })
    const sessionId = String(api.loop.activeSessionId)
  ...
  it('promotes a draft submit into a real session with one-shot title generation (P1-6)', async () => {
    const provider = new FakeProvider()
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-draft-submit-'),
      stateRoot: tmp('cairn-core-api-draft-submit-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(provider),
    })
    const events: Array<Record<string, unknown>> = []
  ...
  it('defers title generation for trivially short first messages and feeds the reply as material (2026-07-05 B7)', async () => {
    const provider = new FakeProvider()
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-short-title-'),
      stateRoot: tmp('cairn-core-api-short-title-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(provider),
    })
  ...
  it('returns distinct workspace and agent state paths for resolved projects', async () => {
    const root = tmp('cairn-core-api-project-paths-')
    const projectDir = tmp('cairn-core-api-project-workspace-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
    ...
    expect(project).toMatchObject({
      ...
      state_path: join(root, '.cairn', 'projects', project.project_id),
      memory_path: join(
        root,
        '.cairn',
        'projects',
  ...
  it('keeps chat and build project contexts isolated in provider prompts', async () => {
    const root = tmp('cairn-core-api-context-isolation-')
    const provider = new FakeProvider()
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
    ...
    expect(
      readFileSync(
        join(root, '.cairn', 'sessions', chatSessionId, 'history.jsonl'),
        'utf8',
      ),
    ).toContain('turn_chat_iso')
    expect(
      readFileSync(
        join(root, '.cairn', 'sessions', String(buildA.id), 'history.jsonl'),
        'utf8',
      ),
    ).toContain('turn_a_iso')
    expect(
      readFileSync(
        join(root, '.cairn', 'sessions', String(buildB.id), 'history.jsonl'),
        'utf8',
      ),
  ...
  it('reconciles stale session pending tags before bootstrap and session list responses', async () => {
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-control-tags-'),
      stateRoot: tmp('cairn-core-api-control-tags-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(new FakeProvider()),
    })
  ...
  it('restores a pending interaction to its declared owner session instead of the active session', async () => {
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-control-owner-'),
      stateRoot: tmp('cairn-core-api-control-owner-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(new FakeProvider()),
    })
    const active = api.loop.sessionStore.get(String(api.loop.activeSessionId))!
  ...
  it('cancels a pending interaction whose declared owner session no longer exists', async () => {
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-control-orphan-'),
      stateRoot: tmp('cairn-core-api-control-orphan-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(new FakeProvider()),
    })
  ...
  it('serves USER.local.md through the config route parity surface', async () => {
    const root = tmp('cairn-core-api-config-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
  ...
  it('exposes a deterministic effective-config snapshot without MCP secrets', async () => {
    const root = tmp('cairn-core-api-effective-config-')
    const stateRoot = join(root, '.cairn')
    const api = await CoreApi.create({
      root,
      stateRoot,
  ...
  it('exposes resumable profile onboarding status, cancellation, and permanent skip', async () => {
    ...
    const root = tmp('cairn-core-api-profile-onboarding-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
  ...
  it('lets the Agent conduct multiple template-driven Ask rounds before saving the profile', async () => {
    const root = tmp('cairn-core-api-profile-dynamic-')
    ...
    await expect(
      api.control.answerInteraction(firstInteractionId, {
        preferred_address: { choice: '自定义称呼', freeform: '老板' },
      }),
    ...
    expect(profile).toContain('- **称呼**：老板')
    ...
    const onboardingState = readFileSync(
      join(root, '.cairn', 'onboarding.json'),
      'utf8',
    )
    expect(onboardingState).not.toContain('老板')
  ...
  it('emits cancelled Plan runtime updates immediately when /plan replaces execution', async () => {
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-plan-replace-event-'),
      stateRoot: tmp('cairn-core-api-plan-replace-event-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(new FakeProvider()),
    })
    const sessionId = String(api.loop.activeSessionId)
  ...
  it('returns structured scheduler payloads for desktop mutations', async () => {
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-scheduler-'),
      stateRoot: tmp('cairn-core-api-scheduler-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(new FakeProvider()),
    })
  ...
  it('redacts internal Scheduler owner and resume fields from public payloads', async () => {
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-scheduler-redaction-'),
      stateRoot: tmp('cairn-core-api-scheduler-redaction-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(new FakeProvider()),
    })
  ...
  it('preserves scheduler payload restrictions at the CoreApi boundary', async () => {
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-scheduler-policy-'),
      stateRoot: tmp('cairn-core-api-scheduler-policy-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(new FakeProvider()),
    })
  ...
  it('applies mutation guard to mcp/model config saves (audit P0-5)', async () => {
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-mutation-guard-'),
      stateRoot: tmp('cairn-core-api-mutation-guard-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(new FakeProvider()),
    })
    api.control.setMode('plan')
  ...
  it('normalizes missing and legacy sidebar state before returning it', async () => {
    const root = tmp('cairn-core-api-sidebar-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
  ...
  it('rejects project workspace APIs for non-Build sessions', async () => {
    const api = await CoreApi.create({
      root: tmp('cairn-core-api-workspace-chat-'),
      stateRoot: tmp('cairn-core-api-workspace-chat-state-'),
      templatesDir: TEMPLATES_DIR,
      modelRouter: fakeRouter(new FakeProvider()),
    })
    const sessionId = String(api.loop.activeSessionId)
  ...
  it('returns diagnostics summaries without mutating missing or corrupt config files', async () => {
    ...
    writeFileSync(
      join(stateRoot, 'cairn.local.json.corrupt-1'),
      '{old broken json',
      'utf8',
    )
    const api = await CoreApi.create({
    ...
    expect(diagnostics.localConfig).toMatchObject({
      path: join(stateRoot, 'cairn.local.json'),
      exists: true,
      status: 'corrupt',
    })
    expect((diagnostics.localConfig as any).corruptBackups).toEqual([
      expect.objectContaining({
        path: join(stateRoot, 'cairn.local.json.corrupt-1'),
      }),
    ])
  ...
  it('reports legacy private data found inside a bound project source tree, without touching it (diagnostics-only)', async () => {
    const root = tmp('cairn-core-api-project-legacy-')
    const projectDir = tmp('cairn-core-api-project-legacy-src-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
    ...
    mkdirSync(join(projectDir, '.cairn', 'sessions'), { recursive: true })
    writeFileSync(
      join(projectDir, '.cairn', 'sessions', 'index.json'),
      '[]',
      'utf8',
    )
    ...
    expect(
      existsSync(join(projectDir, '.cairn', 'sessions', 'index.json')),
    ).toBe(true)
  ...
  it('resumes answered control interactions in their owning session after the user switches away', async () => {
    const root = tmp('cairn-core-api-control-owner-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
    ...
    expect(
      readFileSync(
        join(root, '.cairn', 'sessions', ownerSessionId, 'history.jsonl'),
        'utf8',
      ),
    ).toContain('[CONTROL:ASK_ANSWERED]')
  ...
  it('records cancelled control interactions in their owning session after the user switches away', async () => {
    const root = tmp('cairn-core-api-control-cancel-owner-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
  ...
  it('runs scheduler agent_turn jobs in the session that created them after the user switches away', async () => {
    const root = tmp('cairn-core-api-scheduler-owner-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
  ...
  it('keeps persistent Team roster isolated between build projects', async () => {
    const root = tmp('cairn-core-api-team-project-scope-')
    const projectAPath = join(root, 'project-a')
    const projectBPath = join(root, 'project-b')
    mkdirSync(projectAPath, { recursive: true })
    mkdirSync(projectBPath, { recursive: true })
  ...
  it('controls supervised subagents through wait/read/cancel/resume and fences session ownership', async () => {
    const root = tmp('cairn-core-api-subagent-controls-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
  ...
  it('lists, lease-fences, reparents and cancels only active-session processes', async () => {
    const root = tmp('cairn-core-api-process-controls-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
  ...
  it('cascades session deletion to owned tasks and plans, sparing legacy and other sessions', async () => {
    const root = tmp('cairn-core-api-cascade-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
    ...
    const ownedSidechainDir = join(root, '.cairn', 'tasks', ownedTask.id)
  ...
  it('serves persisted full tool outputs and fences path escapes', async () => {
    const root = tmp('cairn-core-api-toolresult-')
    const api = await CoreApi.create({
      root,
      stateRoot: join(root, '.cairn'),
      templatesDir: TEMPLATES_DIR,
    ...
    const store = new ToolResultStore(join(root, '.cairn'))
  ...
  it('owns slash command discovery and creates a real clear-session boundary', async () => {
    const root = tmp('cairn-core-api-commands-')
    const stateRoot = join(root, '.cairn')
    const api = await CoreApi.create({
      root,
      stateRoot,
  ...
  it('builds the slash command catalog from the current session skill resolution', async () => {
    const root = tmp('cairn-core-api-command-skills-')
    const stateRoot = join(root, '.cairn')
    const projectRoot = tmp('cairn-core-api-command-project-')
    const userSkillDir = join(stateRoot, 'skills', 'greet')
    const projectSkillDir = join(projectRoot, '.cairn', 'skills', 'greet')
