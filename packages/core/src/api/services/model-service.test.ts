...
describe('CoreModelService schema', () => {
  ...
  it('uses modelId as the effective name when no custom alias exists', async () => {
    const root = tmp('cairn-model-service-effective-name-')
  ...
  it('returns and saves explicit model execution policy without exposing credentials', async () => {
    const root = tmp('cairn-model-service-policy-')
    const priced = {
  ...
  it('validates and persists only supported reasoning efforts', async () => {
    const root = tmp('cairn-model-service-reasoning-')
  ...
  it('resolves a draft model profile before an entry is saved', async () => {
    const root = tmp('cairn-model-service-profile-preview-')
    writeConfig(root, [])
    const modelService = await service(root)
  ...
  it('tests exactly the requested entry without accepting a model role', async () => {
    const root = tmp('cairn-model-service-test-')
    writeConfig(
      root,
      [
  ...
  it('discovers models using the selected protocol and restores an existing masked key', async () => {
    const root = tmp('cairn-model-service-discover-')
    writeConfig(root, [
      entry({
        entryId: 'deepseek',
        provider: 'deepseek',
  ...
  it('never reuses an entry credential for a changed discovery endpoint or explicit null key', async () => {
    const root = tmp('cairn-model-service-discover-secret-boundary-')
    writeConfig(root, [
      entry({
        entryId: 'deepseek',
        provider: 'deepseek',
  ...
  it('requests onboarding when activate or delete first transitions the active model to usable', async () => {
    ...
    const activateRoot = tmp('cairn-model-service-activate-onboarding-')
    ...
    const deleteRoot = tmp('cairn-model-service-delete-onboarding-')
  ...
  it('keeps vision tests read-only and rejects descriptive negative answers', async () => {
    const root = tmp('cairn-model-service-vision-readonly-')
  ...
  it('requires an explicit protocol for custom discovery and rejects removed providers', async () => {
    const root = tmp('cairn-model-service-discover-validation-')
    writeConfig(root, [])
    const modelService = await service(root)
