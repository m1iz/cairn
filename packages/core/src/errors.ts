...
export interface CairnErrorOptions extends ErrorOptions {
  action?: string | null
}
...
export class StoreCorruptError extends CairnError {
  /** 被隔离的损坏文件备份路径（若已隔离）。 */
...
/** 模型尚未配置到可用状态。 */
export class ModelConfigurationError extends CairnError {
  constructor(
...
export class ModelProviderError extends CairnError {
...
export class ModelCostCapExceededError extends CairnError {
...
/** 模型输入超过上下文窗口。 */
export class ContextOverflowError extends CairnError {
  constructor(message: string, options?: ErrorOptions) {
...
export function toSafeError(err: unknown): SafeErrorPayload {
  if (err instanceof CairnError) return err.toSafe()
