...
const ENV_KEY = 'CAIRN_CONFIG_DIR'
...
describe('RuntimePaths', () => {
  ...
  it('defaultStateRoot() resolves under the real home directory (string only, no disk access)', () => {
    expect(defaultStateRoot()).toBe(join(homedir(), '.cairn'))
  })

  it('falls back to defaultStateRoot() when neither explicit stateRoot nor CAIRN_CONFIG_DIR is set', () => {
    delete process.env[ENV_KEY]
    const root = tmp('cairn-runtime-paths-')
    ...
    // real directories under the machine's actual ~/.cairn.
    expect(paths.memoryRoot).toBe(join(defaultStateRoot(), 'memory'))
    expect(paths.sessionsRoot).toBe(join(defaultStateRoot(), 'sessions'))
  })

  it('CAIRN_CONFIG_DIR overrides the default when no explicit stateRoot is passed', () => {
    const envStateRoot = tmp('cairn-env-state-root-')
  ...
  it('explicit stateRoot overrides CAIRN_CONFIG_DIR', () => {
    process.env[ENV_KEY] = tmp('cairn-env-state-root-')
    const root = tmp('cairn-runtime-paths-')
    const explicitStateRoot = tmp('cairn-explicit-state-root-')
  ...
  it('runtimeRoot and stateRoot can differ, and resource dirs stay under runtimeRoot', () => {
    const root = tmp('cairn-runtime-root-')
    const stateRoot = tmp('cairn-state-root-')
