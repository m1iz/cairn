...
describe('TaskStore (test_tasks_store.py)', () => {
  it('round-trips records and quarantines corrupt indexes', () => {
    const root = tmp('cairn-task-store-')
  ...
  it('round-trips session ownership and tolerates legacy records without it', () => {
    const root = tmp('cairn-task-session-')
    const store = new TaskStore(root)
  ...
  it('rejects a stale Task revision with compare-and-swap semantics', () => {
    const store = new TaskStore(tmp('cairn-task-cas-'))
  ...
  it('archives only terminal tasks over cap and preserves archived lookup', () => {
    const root = tmp('cairn-task-archive-')
...
describe('TaskManager and SidechainTranscript (test_task_runtime_api.py)', () => {
  it('starts, updates, completes, fails, cancels, and reads sidechain transcripts', () => {
    const root = tmp('cairn-task-manager-')
  ...
  it('stamps session ownership onto started tasks', () => {
    const manager = new TaskManager(tmp('cairn-task-manager-session-'))
  ...
  it('sidechain transcript skips bad lines and returns absolute path', () => {
    const root = tmp('cairn-sidechain-')
  ...
  it('keeps sidechain transcript paths inside the task directory', () => {
    const root = tmp('cairn-sidechain-boundary-')
  ...
  it('rejects a sidechain task directory symlink that escapes tasks', () => {
    const root = tmp('cairn-sidechain-symlink-')
    const outside = tmp('cairn-sidechain-outside-')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const root = tmp('cairn-sidechain-file-symlink-')
      const outside = join(tmp('cairn-sidechain-file-outside-'), 'secret')
...
describe('ProjectStore (test_project_store.py)', () => {
  it('resolves a project path and stores managed memory outside the user project', async () => {
    ...
    const stateRoot = tmp('cairn-project-store-state-')
    const projectDir = tmp('cairn-project-store-workspace-')
  ...
  it('updates project memory through section patches and rejects sectionless overwrites', async () => {
    ...
    const stateRoot = tmp('cairn-project-store-patch-state-')
    const projectDir = tmp('cairn-project-store-patch-workspace-')
    ...
    expect(managed).toContain('npm test --workspace @cairn/core')
  ...
  it('imports a legacy managed AGENTS.md block without mutating the project file', async () => {
    const { ProjectStore, PROJECT_MEMORY_START, PROJECT_MEMORY_END } =
      await import('../projects/store')
    const stateRoot = tmp('cairn-project-store-state-')
    const projectDir = tmp('cairn-project-store-legacy-workspace-')
    const agentsPath = join(projectDir, 'AGENTS.md')
    const legacyText = [
      '# Existing AGENTS',
      '',
  ...
  it('adds project-local collaboration files to build context without treating them as managed memory', async () => {
    const { ProjectStore } = await import('../projects/store')
    const stateRoot = tmp('cairn-project-store-state-')
    const projectDir = tmp('cairn-project-store-context-workspace-')
    writeFileSync(
      join(projectDir, 'AGENTS.md'),
      '# Repo Instructions\n\n- Keep generated files under assets/generated.\n',
      'utf8',
    )
    mkdirSync(join(projectDir, '.cairn', 'rules'), { recursive: true })
    writeFileSync(
      join(projectDir, '.cairn', 'settings.json'),
      JSON.stringify({ formatter: 'prettier' }),
      'utf8',
    )
    writeFileSync(
      join(projectDir, '.cairn', 'rules', 'handoff.md'),
    ...
    expect(agents).toContain('# Workspace .cairn/settings.json')
    expect(agents).toContain('"formatter": "prettier"')
    expect(agents).toContain('# Workspace .cairn/rules/handoff.md')
    expect(store.readManagedMemory(entry.project_id)).not.toContain(
      'Keep generated files under assets/generated',
    )
    expect(existsSync(join(projectDir, '.cairn', 'sessions'))).toBe(false)
    expect(existsSync(join(projectDir, '.cairn', 'memory'))).toBe(false)
  })

  it('rejects missing project directories', async () => {
    const { ProjectStore } = await import('../projects/store')
    const root = tmp('cairn-project-missing-')
