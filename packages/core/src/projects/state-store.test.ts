...
describe('ProjectStateStore', () => {
  it('keeps private project memory under state root and leaves workspace AGENTS.md untouched', () => {
    const stateRoot = tmp('cairn-project-state-')
    const workspace = tmp('cairn-project-workspace-')
  ...
  it('reads allowed project-local collaboration files without writing private state into the workspace', () => {
    ...
    mkdirSync(join(workspace, '.cairn', 'rules'), { recursive: true })
    writeFileSync(
      join(workspace, '.cairn', 'settings.json'),
      JSON.stringify({ style: 'quiet' }),
      'utf8',
    )
    writeFileSync(
      join(workspace, '.cairn', 'settings.local.json'),
      JSON.stringify({ local: true }),
      'utf8',
    )
    writeFileSync(
      join(workspace, '.cairn', 'rules', 'build.md'),
    ...
    expect(context).toContain('# Workspace .cairn/settings.json')
    expect(context).toContain('"style": "quiet"')
    expect(context).toContain('# Workspace .cairn/rules/build.md')
    expect(context).toContain('Run make check before handoff')
    expect(existsSync(join(workspace, '.cairn', 'sessions'))).toBe(false)
    expect(existsSync(join(workspace, '.cairn', 'memory'))).toBe(false)
  })
})
