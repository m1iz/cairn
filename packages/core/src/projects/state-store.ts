...
export class ProjectStateStore {
  ...
  readWorkspaceCollaborationContext(projectId: string): string {
    ...
    pushJsonSection(
      sections,
      'Workspace .cairn/settings.json',
      join(workspacePath, '.cairn', 'settings.json'),
    )
    pushJsonSection(
      sections,
      'Workspace .cairn/settings.local.json',
      join(workspacePath, '.cairn', 'settings.local.json'),
    )
    for (const rule of listMarkdownFiles(
      join(workspacePath, '.cairn', 'rules'),
    )) {
      pushTextSection(
        sections,
        `Workspace .cairn/rules/${rule}`,
        join(workspacePath, '.cairn', 'rules', rule),
      )
    }
