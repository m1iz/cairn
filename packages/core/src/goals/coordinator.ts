...
export class GoalCoordinator {
  ...
  private harnessHost: HarnessHost<GoalCycleTurnInput> | null
  private launchBarrier: Promise<void> = Promise.resolve()

  constructor(private readonly options: GoalCoordinatorOptions) {
    this.now = options.now ?? (() => new Date().toISOString())
    this.harnessHost = options.runTurn
      ? new CallbackHarnessHost(options.runTurn)
      : null
  }

  setHarnessHost(host: HarnessHost<GoalCycleTurnInput> | null): void {
    this.harnessHost = host
  }
  ...
  private async runLoop(
    ...
    while (!signal.aborted) {
      ...
      const host = this.harnessHost
      if (!host) throw new Error('Goal turn submitter is not configured.')
