...
describe('ContextBuilder (test_agent_prompt_contracts.py — template-driven)', () => {
  ...
  it('assembles bootstrap + identity phrases', () => {
    ...
    expect(prompt).toContain('update_long_term_memory')
    expect(prompt).toContain('delete_long_term_memory')
    expect(prompt).toContain('“这次/当前对话”只影响当前会话')
  ...
  it('keeps a warmer collaboration style behind the explicit classic prompt profile', () => {
    const builder = new ContextBuilder(TEMPLATES_DIR, EMPTY_SKILLS, {
    ...
    expect(prompt).toContain('温和、有个性的协作口吻')
  })

  it('loads USER.local.md from the private state root when provided', () => {
    const stateTemplates = mkdtempSync(join(tmpdir(), 'cairn-user-template-'))
