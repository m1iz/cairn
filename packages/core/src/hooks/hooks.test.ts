...
describe('hooks schema, audit, and matching', () => {
  ...
  it('preserves corrupt audit lines while replaying newest bounded records', async () => {
    const root = tmp('cairn-hooks-audit-')
    ...
    await store.append({
      ...
      source: {
        kind: 'project',
        path: join(root, '.cairn/settings.json'),
        readonly: true,
      },
  ...
  it('uses deterministic defaults for empty configs', () => {
    ...
    expect(readdirSync(tmp('cairn-hooks-empty-'))).toEqual([])
  })
})
...
describe('hooks config loading, execution, and decision aggregation', () => {
  it('loads editable global hooks and readonly project hooks only when trusted', async () => {
    const stateRoot = tmp('cairn-hooks-config-state-')
    const projectRoot = tmp('cairn-hooks-config-project-')
  ...
  it('preserves corrupt global config and reports malformed project hooks without writing project files', async () => {
    const stateRoot = tmp('cairn-hooks-corrupt-state-')
    const projectRoot = tmp('cairn-hooks-corrupt-project-')
    mkdirSync(join(projectRoot, '.cairn'), { recursive: true })
    ...
    expect(
      readFileSyncText(join(projectRoot, '.cairn/settings.json')),
    ).toContain('"mcp"')
  })
  ...
  it('runs matching hooks through runtime events, audit, and aggregate decisions', async () => {
    const stateRoot = tmp('cairn-hooks-runtime-state-')
