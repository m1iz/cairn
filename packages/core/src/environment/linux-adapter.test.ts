...
describe('LinuxEnvironmentAdapter', () => {
  ...
  it('rejects a symlinked managed root before touching external data', async () => {
    ...
    const outside = mkdtempSync(join(tmpdir(), 'cairn-linux-outside-'))
  ...
  it('rejects an activation directory that escapes the user home', async () => {
    ...
    const outside = mkdtempSync(join(tmpdir(), 'cairn-linux-bin-outside-'))
  ...
  it('reports AppImage FUSE constraints without trying to install them', () => {
    expect(
      linuxAppImageDiagnostics({
        env: { APPIMAGE: '/opt/Cairn.AppImage' },
        pathExists: () => false,
      }),
...
function fixtureAdapter(
  ...
  const root = opts.root ?? mkdtempSync(join(tmpdir(), 'cairn-linux-'))
...
function assetCatalog(
  ...
  const root = mkdtempSync(join(tmpdir(), 'cairn-linux-asset-'))
